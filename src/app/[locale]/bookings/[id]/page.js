// src/app/[locale]/bookings/[id]/page.js
"use client";
...
/**
 * Booking Details — locale-aware version
 */
export default function BookingDetailsPage({ params }) {
  ...
  // ---------- QR support ----------
  const qrValue = useMemo(() => getQrValue(booking, locale), [booking, locale]);
  const shouldShowQr =
    !!dateObj &&
    isValid(dateObj) &&
    !!qrValue &&
  ...
  useEffect(() => {
    if (!copied) return;
    const tId = setTimeout(() => setCopied(false), 1500);
    ...
    <div className="min-h-screen bg-[#fcfaf7] text-[#3d3d3d]">
      ...
      <main className="max-w-5xl mx-auto pt-20 sm:pt-24 px-4 sm:px-6 pb-28">
        ...
        <div className="mb-5 flex items-center justify-between gap-3">
          <div className="flex items-center gap-3">
            ...
            >
              <ChevronLeft className="w-4 h-4" /> {t("backToBookings")}
            </Link>
            {booking?.id && (
              <span className="hidden sm:inline-flex items-center gap-1 text-xs rounded-full bg-[#f3efe8] px-2 py-1 border border-[#e8e2d9]">
                ...
                <span>
                  {t("refLabel", { ref: getPublicBookingRef(booking) })}
                </span>
              </span>
            )}
          </div>
        ...
        {error ? (
          <ErrorCard message={error} locale={locale} />
        ) : (
              ...
              <div className="lg:col-span-2 bg-white/90 backdrop-blur border border-[#e4ddd3] rounded-2xl p-6 shadow-md">
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 text-sm text-[#5a4a3f]">
                  ...
                  >
                    {dateObj ? (
                      <span>
                        {format(dateObj, "PPPP", { locale: dateLocale })}{" "}
                        <span className="text-[#7c6f60]">
                          {t("atTime", {
                            time: format(dateObj, "p", { locale: dateLocale }),
                          })}
                        </span>{" "}
                        (
                        <span>
                          {t("durationMinutes", { count: durationMin })}
                        </span>
                        )
                      </span>
                    ) : (
                  ...
                  <InfoRow icon={<Users className="w-5 h-5 text-[#8b6f47]" />}>
                    <span>{t("peopleCount", { count: people })}</span>
                  </InfoRow>
                ...
                {(booking?.totalPaidAmount ||
                  ...
                  <div className="mt-6 pt-4 border-t border-[#eee7dc] flex items-center justify-between">
                    <span className="text-[#7c6f60] text-sm">
                      {t("totalPaid")}
                    </span>
                ...
                <div className="mt-6 flex flex-wrap gap-3">
                  ...
                  {exp?.location && (
                    ...
                    >
                      <Navigation className="w-4 h-4" /> {t("directionsCta")}
                    </a>
                  )}
                  ...
                  {canShare() && (
                    <button
                      onClick={() =>
                        shareBooking(booking, exp, dateObj, t, dateLocale)
                      }
      ...
      {!error && (
        <MobileToolbar
          ...
          mapHref={mapHref}
          share={() =>
            canShare() && shareBooking(booking, exp, dateObj, t, dateLocale)
          }
        />
      )}
    </div>
...
function formatMoney(amount, currency = "EUR", locale) {
  try {
...
function shareBooking(booking, exp, dateObj, t, dateLocale) {
  if (!canShare()) return;
  const title = exp?.name || t("shareTitleFallback");
  const text = dateObj
    ? t("shareTextWithDate", {
        title,
        datetime: format(dateObj, "PPPP p", { locale: dateLocale }),
      })
    : title;
...
function getQrValue(b, locale) {
  ...
  const locSegment = locale ? `/${locale}` : "";
  return `${base.replace(
    /\/+$/,
    ""
  )}${locSegment}/bookings/${encodeURIComponent(ref)}`;
}
...
function ErrorCard({ message, locale }) {
  const t = useTranslations("BookingDetails");
  return (
    ...
    return (
      <div className="bg-white/70 border border-[#e4ddd3] rounded-2xl p-5 shadow-sm">
        <div className="flex items-start gap-3">
          ...
          <div>
            <p className="font-medium text-[#5a4a3f]">
              {t("checkInUnavailableTitle")}
            </p>
            <p className="text-sm text-[#7c6f60]">
              {t("checkInUnavailableBody")}
            </p>
          </div>
        </div>
      </div>
    ...
    >
      <div className="flex items-center justify-between mb-4">
        ...
        <span className="inline-flex items-center gap-1 text-xs rounded-full bg-[#f1efe8] px-2 py-0.5 border border-[#e8e2d9]">
          ...
          {ongoing
            ? t("checkInChipOpenNow")
            : startsAt && isValid(startsAt)
            ? t("checkInChipOpensAtTime", {
                time: format(startsAt, "p", { locale: dateLocale }),
              })
            : t("checkInChipOpensAtStart")}
        </span>
      </div>
          ...
          <div className="flex items-center gap-1.5">
            ...
            {canRenderQrImage && (
              <button
                ...
                title={t("viewLarger")}
              >
                <Maximize2 className="w-3.5 h-3.5" /> {t("viewLarger")}
              </button>
            )}
            <button
              onClick={() =>
              ...
              className="inline-flex items-center gap-1 rounded-full border border-[#e8e2d9] bg-white px-2 py-1 hover:bg-[#f6f1e6]"
              title={t("printTicketButton")}
            >
              <Printer className="w-3.5 h-3.5" /> {t("printTicketButton")}
            </button>
          </div>
        </div>
      </div>
      ...
      <ul className="mt-4 space-y-2 text-xs text-[#7c6f60]">
        <li className="flex items-start gap-2">
          ...
          {endsAt && isValid(endsAt)
            ? t("checkInFooterUntil", {
                time: format(endsAt, "p", { locale: dateLocale }),
              })
            : t("checkInFooterFallback")}
        </li>
      </ul>
    </div>
  );
...
function printTicket({ qrDataUrl, codeText, startsAt, endsAt, dateLocale }) {
  const when =
    startsAt && isValid(startsAt)
      ? `${format(startsAt, "PPPP p", { locale: dateLocale })}`
      : "";
  const until =
    endsAt && isValid(endsAt)
      ? `${format(endsAt, "p", { locale: dateLocale })}`
      : "";
  ...
  return (
    <div className="lg:hidden fixed bottom-0 left-0 right-0 z-20 bg-white/95 backdrop-blur border-t border-[#e4ddd3]">
      <div className="max-w-5xl mx-auto px-4 py-3 grid grid-cols-4 gap-2 text-xs">
        ...
        >
          <Download className="w-4 h-4" /> {t("toolbarIcs")}
        </button>
        <a
          href={gcalHref}
          target="_blank"
        ...
        >
          <ExternalLink className="w-4 h-4" /> {t("toolbarGcal")}
        </a>
        <a
          href={mapHref}
          target="_blank"
          ...
          className="inline-flex items-center justify-center gap-1 rounded-full border border-[#e8e2d9] bg-white px-3 py-2 text-[#5a4a3f]"
        >
          <Navigation className="w-4 h-4" /> {t("toolbarGo")}
        </a>
        <button
          onClick={share}
          className="inline-flex items-center justify-center gap-1 rounded-full border border-[#e8e2d9] bg-white px-3 py-2 text-[#5a4a3f]"
        >
          <Share2 className="w-4 h-4" /> {t("toolbarShare")}
        </button>
      </div>
    </div>
  );
