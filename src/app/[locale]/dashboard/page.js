...
export default function Dashboard() {
  ...
  const greetingName =
    [finalProfile.first].filter(Boolean).join(" ").trim() || t("fallbackName");

  const timeGreetingKey = useMemo(() => getTimeGreetingKey(), []);
  const timeGreeting = t(timeGreetingKey);
  ...
  }, [user, loading, profileLoading, router, locale]);

  if (loading || profileLoading || !user) return <Skeleton />;
  ...
  function calculateMemberStatusKey(createdAt) {
    if (!createdAt) return "memberStatus.member";
    const now = new Date();
    const created = new Date(createdAt);
    const diffDays =
      (now.getTime() - created.getTime()) / (1000 * 60 * 60 * 24);
    if (diffDays < 30) return "memberStatus.newcomer";
    if (diffDays >= 365) return "memberStatus.loyal";
    return "memberStatus.member";
  }

  function getBadgeLabel(badgeRaw) {
    const norm = String(badgeRaw || "").toLowerCase();
    if (norm === "admin") return t("badge.admin");
    if (norm === "explorer") return t("badge.explorer");
    return badgeRaw || t("badge.default");
  }
  ...
  async function handleDeleteAccount() {
    ...
    try {
      ...
      if (res.ok) {
        ...
        // Always go to localized goodbye page on successful deletion
        router.replace(`/${locale}/goodbye`);
        return;
      }
      ...
      setMessage(t("deleteGenericError"));
      setIsError(true);
    } finally {
      setIsDeleting(false);
      setIsModalOpen(true);
  ...
  const memberStatusKey = calculateMemberStatusKey(finalProfile.createdAt);
  const memberStatus = t(memberStatusKey);
  const badgeLabel = getBadgeLabel(finalProfile.badge);
          ...
          <div className="flex flex-col gap-6 sm:flex-row sm:items-center">
              ...
              <div className="min-w-0">
                ...
                <div className="mt-4 flex flex-wrap items-center gap-2">
                  <Chip
                    ...
                    label={`${memberStatus} • ${badgeLabel}`}
                  />
                  {finalProfile.role === "admin" && (
                    <Chip label={t("badgeAdminAccess")} />
                  )}
                  {hasActiveReservations && (
                    <Chip label={t("chipActiveReservations")} tone="accent" />
                  )}
                </div>
              </div>
            </div>
            ...
            <div className="w-full max-w-xs rounded-2xl border border-[#e1dad1] bg-white/85 px-4 py-4 shadow-sm backdrop-blur-sm sm:w-64">
              ...
              <div className="space-y-1.5 text-sm text-[#5a4a3f]">
                <SnapshotRow
                  label={t("snapshotMembershipLabel")}
                  value={memberStatus}
                />
                <SnapshotRow
                  label={t("snapshotStatusLabel")}
                  value={
                    hasActiveReservations
                      ? t("snapshotStatusUpcoming")
                      : t("snapshotStatusNone")
                  }
                />
                <SnapshotRow
                  label={t("snapshotMemberSinceLabel")}
                  value={
          ...
          <div className="space-y-6 lg:col-span-2">
            ...
            <div className="rounded-3xl border border-[#e0dcd4] bg:white/90 bg-white/90 p-6 shadow-[0_18px_45px_rgba(93,71,43,0.06)] backdrop-blur">
              ...
              <div className="mt-5 grid grid-cols-1 gap-4 sm:grid-cols-2">
                ...
                />
                <Row
                  label={t("detailsEmailLabel")}
                  value={finalProfile.email || t("detailsNotProvided")}
                  mono
                />
                <Row
                  label={t("detailsPhoneLabel")}
                  value={finalProfile.phone || t("detailsNotProvided")}
                  mono
                />
                <Row
                  label={t("detailsDobLabel")}
                  value={
                ...
                />
                <Row
                  label={t("detailsMemberSinceLabel")}
                  value={
                    finalProfile.createdAt
                      ? new Date(finalProfile.createdAt).toLocaleDateString(
                          dateLocale
                  ...
                  mono
                />
                <Row label={t("detailsBadgeLabel")} value={badgeLabel} />
              </div>
            ...
            <div className="grid grid-cols-1 gap-5 sm:grid-cols-2">
              <ActionCard
                ...
                label={t("actionBookingsLabel")}
                description={t("actionBookingsDescription")}
                onClick={() => handleRedirect("/bookings")}
                t={t}
              />
              <ActionCard
                icon={<Settings size={26} />}
                label={t("actionSettingsLabel")}
                description={t("actionSettingsDescription")}
                onClick={() => handleRedirect("/account/settings")}
                t={t}
              />
            </div>
          ...
          <aside className="space-y-6">
            <div className="rounded-3xl border border-[#e1dad1] bg-[#fdfaf7] p-5 text-sm text-[#5a4a3f] shadow-sm">
              ...
              <ul className="mt-3 space-y-1.5 text-xs text-[#7a6a5f]">
                <li>• {t("accountPrivacyBullet1")}</li>
                <li>• {t("accountPrivacyBullet2")}</li>
              </ul>
            </div>
            ...
            <div className="rounded-3xl border border-red-100 bg-red-50/65 p-5 text-sm shadow-sm">
              ...
              >
                <Trash2 size={16} />
                {hasActiveReservations
                  ? t("deleteButtonUpcoming")
                  : t("deleteButtonDefault")}
              </button>

              <p className="mt-2 text-[11px] leading-snug text-[#9a6b64]">
                {t("deleteSectionFooter")}
              </p>
            </div>
          </aside>
        </div>
...
function ActionCard({ icon, label, description, onClick, t }) {
  return (
    <button
      onClick={onClick}
...
function getTimeGreetingKey(date = new Date()) {
