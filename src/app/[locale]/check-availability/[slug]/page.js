...
export default function CheckAvailabilityPage() {
  ...
  // Locale & translations from next-intl
  const t = useTranslations("CheckAvailability");
  const locale = useLocale();
  const dateLocale = locale === "el" ? el : enGB;

  // your dynamic segment (you used slug)
  ...
  async function handleContinue() {
    ...
    try {
      ...
      router.push(
        `/${locale}/booking/${data.id}/attendees${qs ? `?${qs}` : ""}`
      );
    } catch (e) {
      console.error(e);
    ...
    <main className="relative min-h-screen bg-gradient-to-b from-[#f4f1ec] via-[#faf9f7] to-[#f4f1ec] text-[#2f2f2f] overflow-x-clip">
          ...
          <div className="flex items-center justify-between pb-4">
            ...
            {experience?.name && (
              <div className="hidden sm:flex items-center gap-2 text-[#6b5e53] text-sm">
                ...
                {t("maxPerBooking", { max: MAX_PER_BOOKING })}
              </div>
            )}
          </div>
        </div>
        ...
        <div className="mx-auto max-w-6xl px-4 sm:px-6">
          <div className="relative overflow-hidden rounded-[2rem] border border-[#e5e0d8] bg-[#fcf9f4] shadow-[0_16px_40px_rgba(90,74,63,0.12)]">
            ...
            {firstImage ? (
              ...
              <img
                src={firstImage}
                alt={experience?.name || t("experienceFallback")}
            ...
            <div className="relative z-10 p-6 sm:p-8">
                ...
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    <h1 className="text-2xl sm:text-3xl md:text-4xl font-serif text-[#5a4a3f] leading-tight">
                      {experience?.name || t("experienceFallback")}
                    </h1>
                    {experience?.location ? (
              ...
              <div className="mt-6">
                <div className="grid grid-cols-3 gap-2 text-xs sm:text-sm">
                  <Step
                    label={t("stepChooseDate")}
                    active={step >= 1}
                    done={step > 1}
                  />
                  <Step
                    label={t("stepChooseTime")}
                    active={step >= 2}
                    done={step > 2}
                  />
                  <Step label={t("stepGroupSize")} active={step >= 3} />
                </div>
              ...
              {settingsLoading ? (
                ...
                <div className="mt-4 flex items-start gap-2 rounded-xl border border-[#f1d7d7] bg-[#fff6f6] px-3 py-2 text-xs text-[#7a4a4a] shadow-sm">
                  ...
                  <div>
                    <p className="font-medium">{t("bookingsPausedTitle")}</p>
                    {globalSettings.bookingsPausedUntil ? (
                      <p>
                        {t("bookingsPausedResumeAfter", {
                          date: format(
                            new Date(globalSettings.bookingsPausedUntil),
                            "PPpp",
                            { locale: dateLocale }
            ...
            <section className="space-y-6" ref={slotsContainerRef}>
              ...
              >
                ...
                {!selectedDate ? (
                  ...
                  <p className="text-sm text-[#7a6a5a]">{t("timesNoSlots")}</p>
                ) : (
                  <div
                    ref={timesListRef}
                    role="radiogroup"
                    aria-label={t("availableStartTimes")}
                  ...
                  >
                    {slotsOnSelectedDay.map((slot) => {
                      ...
                      const availableLabel =
                        available <= 0
                          ? t("slotFullyBooked")
                          : t("slotAvailablePlural", { count: available });
                      ...
                      return (
                        <button
                          ...
                          aria-label={`${time}, ${availableLabel}`}
                        >
                          <div>
                ...
                <div className="rounded-3xl border border-[#e8e5df] bg-[#fcf9f4]/95 p-6 shadow-[0_12px_32px_rgba(90,74,63,0.12)]">
                  ...
                  <div className={`mt-4 ${pausedNow ? "opacity-60" : ""}`}>
                    <div className="flex items-center justify-between">
                      <p className="text-xs text-[#7a6a58]">{t("ageHint")}</p>
                      {(adults !== 1 || kids !== 0) && (
                        <button
                          type="button"
                    ...
                    <div className="mt-3 grid grid-cols-1 sm:grid-cols-3 gap-4">
                      ...
                      <div className="bg-white border border-[#e2ddd2] rounded-xl p-3 shadow-sm flex flex-col justify-center">
                        <div className="text-sm text-[#5a4a3f] mb-1">
                          {t("total")}
                        </div>
                    ...
                    {selectedSlot && (
                      <p className="mt-2 text-xs text-[#5a4a3f]">
                        {t("groupSummary", {
                          selected: totalPeople,
                          remaining: Math.max(0, bookingCap - totalPeople),
                          cap: bookingCap,
                          max: MAX_PER_BOOKING,
                        })}
                      </p>
      ...
      <div className="sm:hidden fixed bottom-0 left-0 right-0 border-t border-[#e5e0d8] bg-white/95 backdrop-blur supports-[backdrop-filter]:bg-white/80 px-4 py-3">
        <div className="mx-auto max-w-6xl flex items-center justify-between gap-3">
          <div>
            <div className="text-xs text-[#7a6a58]">{t("total")}</div>
