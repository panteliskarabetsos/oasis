...
export async function POST(req) {
  try {
    ...
    if (ctype.includes("application/json")) {
      ...
      paymentIntentId =
        fd.get("paymentIntentId") || fd.get("piId") || undefined;
    ...
    if (!paymentIntentId && !chargeId && bookingId) {
      ...
      const { data: b, error } = await admin
        .from("booking")
        .select(
          "id, stripePaymentIntentId, stripeSessionId, currency, totalPaidAmount"
        )
        .eq("id", bookingId)
        .single();
      ...
      if (b.stripePaymentIntentId) {
        ...
        if (!paymentIntentId && sess.payment_intent?.latest_charge) {
          chargeId =
            typeof sess.payment_intent.latest_charge === "string"
        ...
        return NextResponse.json(
          { error: "not_refundable_out_of_band" },
          { status: 400 }
        );
      }
    }
        ...
        if (admin) {
          await admin
            ...
            .update({
              ...
              lastRefundAmount:
                (refund.amount ?? 0) /
                (ZERO_DEC.has((refund.currency || "").toUpperCase()) ? 1 : 100),
