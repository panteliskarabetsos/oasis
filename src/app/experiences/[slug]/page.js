...
import { ArrowLeft, CheckCircle2, AlertCircle, ChevronLeft, ChevronRight, X } from "lucide-react";
...
export async function generateMetadata({ params }) {
  ...
  const url = `${siteUrl}/experiences/${slug}`; // fixed plural
...
export default async function ExperienceDetailPage({ params }) {
  ...
  return (
    <main className="bg-[#f4f1ec] text-[#2f2f2f]">
      ...
      <Script id="experience-jsonld" type="application/ld+json" dangerouslySetInnerHTML={{ __html: JSON.stringify(jsonLd) }} />

      {/* Decorative ambient background */}
      <div aria-hidden className="fixed inset-0 -z-10">
        <div className="pointer-events-none absolute -top-32 left-1/2 h-[36rem] w-[36rem] -translate-x-1/2 rounded-full blur-3xl opacity-40" style={{
          background: "radial-gradient(closest-side, #e8dfcf, transparent 70%)",
        }} />
      </div>

      {/* Back button overlay + Hero section (user-preferred) */}
        ...
        {parsedImages?.[0] ? (
          <>
            <Image
              ...
              sizes="100vw"
            />
            {/* Fade to page background for a smooth transition */}
        ...
        <div className="absolute inset-0 flex items-end justify-center">
          <div className="mx-auto w-full max-w-6xl px-6 pb-6">
            <div className="backdrop-blur-sm bg-white/50 border border-white/50 rounded-2xl shadow-lg p-5 sm:p-7">
              ...
              {(duration || typeof price !== "undefined" || location) && (
      ...
      <div className="mx-auto max-w-7xl px-4 sm:px-6">
          ...
          <section className="pb-12 sm:pb-16">
            <div className="grid grid-cols-1 gap-6 sm:gap-8 md:grid-cols-3">
              {whatsIncluded && (
                <div className="col-span-1 md:col-span-2 rounded-3xl border-2 border-[#e0dcd4] bg-white/80 p-6 sm:p-8 shadow-[0_10px_30px_rgba(0,0,0,0.06)]">
                  <h3 className="text-2xl sm:text-3xl font-serif text-[#5a4a3f] mb-6">What’s Included</h3>
                  <ul className="grid grid-cols-1 sm:grid-cols-2 gap-y-3 gap-x-6">
                    {splitItems(whatsIncluded).map((item, i) => (
                      <li key={i} className="inline-flex items-start gap-2 text-[17px] text-[#4a4a4a]">
              ...
              {whatToBring && (
                <div className="rounded-3xl border-2 border-[#e0dcd4] bg-white/80 p-6 sm:p-8 shadow-[0_10px_30px_rgba(0,0,0,0.06)]">
                  <h3 className="text-2xl sm:text-3xl font-serif text-[#5a4a3f] mb-4 inline-flex items-center gap-2"><AlertCircle size={22} aria-hidden /> What to Bring</h3>
                  <p className="text-[#4a4a4a] whitespace-pre-line">{whatToBring}</p>
                </div>
              )}
            </div>
            ...
            {whyYoullLove && (
              <div className="mt-8 rounded-3xl border-2 border-[#e0dcd4] bg-[#fffdf9] p-6 sm:p-10 shadow-[0_10px_30px_rgba(0,0,0,0.06)]">
                <h3 className="text-2xl sm:text-3xl font-serif text-[#5a4a3f] mb-4 text-center">Why You’ll Love It</h3>
        ...
        {parsedImages.length > 1 && (
          <section className="pb-16">
            <h3 className="text-2xl sm:text-3xl font-serif text-[#5a4a3f] mb-6 text-center">Gallery</h3>

            {/* Thumbnail grid (click to open slideshow) */}
            ...
            {/* Lightbox slideshow (CSS-only using :target) */}
            {(() => {
              ...
              return parsedImages.slice(1).map((img, idx) => {
                ...
                return (
                  <div key={`overlay-${j}`} id={`lb-${j}`} className="lightbox fixed inset-0 z-[60] hidden">
                    {/* Backdrop (click to close) */}
                    <a href="#_" className="absolute inset-0 bg-black/80" aria-label="Close" />
            ...
            {/* Plain <style> tag (no styled-jsx) to avoid Client Component requirement */}
            <style>{`
        ...
        {mapPin && (
          <section className="pb-20">
            <h3 className="text-2xl sm:text-3xl font-serif text-[#5a4a3f] mb-6 text-center">Where You'll Be</h3>
            <div className="overflow-hidden rounded-2xl border-2 border-[#e0dcd4] shadow-lg">
              <div className="w-full aspect-[16/9]">
                <iframe
                  title={`Map location for ${name}`}
                  src={`https://www.google.com/maps?q=${encodeURIComponent(mapPin)}&z=14&output=embed`}
                  width="100%"
                  height="100%"
                  style={{ border: 0 }}
                  allowFullScreen
            ...
            <p className="mt-2 text-center text-sm text-[#4a4a4a]">
              ...
              <a
                href={`https://www.google.com/maps/search/?api=1&query=${encodeURIComponent(mapPin)}`}
                target="_blank"
        ...
        {parsedReviews.length > 0 && (
          <section className="pb-24">
            <h3 className="text-2xl sm:text-3xl font-serif text-[#5a4a3f] mb-6 text-center">Guest Reviews</h3>
            <div className="grid gap-6 sm:grid-cols-2">
              {parsedReviews.map((review, i) => (
                <article key={(review?.name || "guest") + i} className="rounded-2xl border-2 border-[#e0dcd4] bg-white p-6 shadow-xl">
                  <div className="flex items-center gap-3">
                    ...
                    <p className="font-semibold text-lg text-[#5a4a3f]">{review?.name || "Guest"}</p>
                  </div>
                  {review?.comment && (
                    <p className="mt-3 italic text-[#4a4a4a]">“{review.comment}”</p>
                  )}
                </article>
              ))}
            </div>
...
function buildJsonLd({ name, description, price, location, images, pageUrl }) {
  const offers =
    ...
    <main className="min-h-screen flex items-center justify-center text-center px-6 bg-[#f4f1ec] text-[#5a4a3f]">
      <div className="max-w-md">
        ...
        <p className="text-lg mb-6">
          It might be private or has been removed. Please explore our other unique offerings.
        </p>
        <Link href="/experiences" className="inline-block rounded-full bg-[#8b6f47] px-6 py-3 text-white font-medium hover:bg-[#a78b62] transition-all">
          Browse Experiences
        </Link>
      </div>
    </main>
