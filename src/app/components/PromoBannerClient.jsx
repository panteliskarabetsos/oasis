...
/** One-line marquee that activates only when content overflows */
function OneLineMarquee({ children, paused, reduced, speedPxPerSec = 48 }) {
  ...
  // Measure overflow and compute animation distance + duration
  useEffect(() => {
    const el = containerRef.current;
    const inner = contentRef.current;
    if (!el || !inner) return;

    const GAP = 48; // space between duplicated contents during scroll
    ...
    const measure = () => {
      ...
      setActive(needs);
      if (needs) {
        const dist = contentW + GAP; // scroll full content width + gap
        setDistancePx(dist);
        setDurationSec(Math.max(6, dist / speedPxPerSec)); // cap minimum for readability
      }
    };
  ...
  return (
    ...
    >
      <div
        className={[
          "flex items-center",
          animate ? "promo-marquee-track promo-marquee-animate" : "",
          paused ? "promo-marquee-paused" : "",
        ].join(" ")}
        style={
          animate
            ? {
      ...
      >
        ...
        {animate && (
          <div aria-hidden className="flex shrink-0 items-center gap-2 ml-12">
            {children}
          </div>
        )}
      </div>

      {/* Optional soft edge fades when animating */}
      {animate && (
        <>
          <div className="pointer-events-none absolute inset-y-0 left-0 w-6 bg-gradient-to-r from-[#f6f2ea] to-transparent dark:from-zinc-900" />
          <div className="pointer-events-none absolute inset-y-0 right-0 w-6 bg-gradient-to-l from-[#f6f2ea] to-transparent dark:from-zinc-900" />
        </>
      )}
      ...
      <style jsx>{`
        ...
        .promo-marquee-animate {
          animation: promoMarqueeScroll var(--marquee-duration) linear infinite;
        }
        .promo-marquee-paused {
 ...
 * - Long strings are fully shown via a one-line marquee (no truncation)
 */
    ...
    >
      <div className="mx-auto max-w-6xl px-3">
        <div className="relative py-2.5 flex items-center justify-center min-h-[1.75rem]">
          ...
          {/* One-line centered content (with marquee for long text) */}
          {/* Centered row using 3-column grid (left empty now) */}
          <div className="grid grid-cols-[1fr_auto_1fr] items-center w-full text-[12px] leading-none">
            {/* Left spacer to keep center perfectly centered */}
            <div />

            {/* Center: dot + marquee content */}
            <div className="min-w-0 justify-self-center text-center w-full">
              <OneLineMarquee paused={paused} reduced={reduced}>
                <div className="inline-flex items-center">
                  <span
                    aria-hidden
                    className="inline-block h-1 w-1 rounded-full bg-[#8b6f47] mr-2"
                  />
                  {current.kind === "code" ? (
                    <div className="inline-flex items-center gap-2 min-w-0">
                      <strong className="font-medium text-[#4b4136]">
