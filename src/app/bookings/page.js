        ...
        {fetching ? (
          ...
          <ErrorCard
            ...
            onRetry={() => {
              setError("");
              setFetching(true);
              /* re-run by toggling user? simplest: hard reload */ window.location.reload();
            }}
          />
        ) : bookings.length === 0 ? (
          ...
          <div className="space-y-12">
            {tab === "upcoming" ? (
              <Section blockTitle="Upcoming Reservations">
                {upcomingBookings.length > 0 ? (
                  ...
                  <MutedCenter>No upcoming reservations.</MutedCenter>
                )}
              </Section>
            ) : (
              <Section blockTitle="Past Reservations">
                {pastBookings.length > 0 ? (
                  ...
                  <MutedCenter>No past reservations yet.</MutedCenter>
                )}
              </Section>
            )}
          </div>
...
function BookingCard({ booking, status }) {
  ...
  const dateStr = date ? format(date, "PPPP") : "—";
  const timeStr = date ? format(date, "p") : "";
  ...
  return (
    <div
      ...
      onKeyDown={(e) =>
        e.key === "Enter" && router.push(`/bookings/${booking.id}`)
      }
    ...
    >
      ...
      <div className="mt-6 flex items-center justify-between">
        <span className="text-xs text-[#9b8f7f]">ID: {booking.id}</span>
