...
import { accessCan } from "@/lib/auth/requireAdmin";
 ...
 *
 * Lifted out of the checkout route so the Stripe webhook can settle a QR
 * payment the till never got to confirm — a customer who has paid must end up
 * with a receipt whether or not the app was still on screen. Both callers run
 * exactly this code, so the two paths cannot drift.
 *
 * Returns `{ status, body }` rather than a Response; the route wraps it.
 *
 * @param {object} body                the POS checkout payload
 * @param {object} opts
 * @param {"*"|string[]} opts.permissions  caller's access, for the comp check
 */
export async function recordPosSale(body, { permissions = [] } = {}) {
  try {
    ...
    if (method === "comp" && !accessCan(permissions, "comps")) {
      return { status: 403, body: { error: "Only a Super Admin can record a complimentary sale." } };
    }
    ...
    if (experienceId && transactionType === "experience") {
      ...
      if (!exp) {
        return { status: 400, body: { error: "Experience not found" } };
      }
    ...
    if (isStripeRail) {
      ...
      if (!piId) {
        return { status: 400, body: { error: "Missing stripePaymentIntentId" } };
      }
      ...
      if (!okStatuses.includes(pi.status)) {
        return { status: 400, body: { error: `PaymentIntent not payable: ${pi.status}` } };
      }
    ...
    if (transactionType === "items" || transactionType === "addons") {
      ...
      if (insertErr) {
        return { status: 400, body: { error: insertErr.message } };
      }
    ...
    return {
      status: 200,
      body: receiptEmail ? { ...responseId, receiptEmail } : responseId,
    };
  } catch (e) {
    return { status: 400, body: { error: e.message || "Checkout error" } };
  }
}
